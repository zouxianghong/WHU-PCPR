...
class OxfordDataset(Dataset):
    ...
    Dataset wrapper for Oxford laser scans dataset from PointNetVLAD project.
    """
    def __init__(self, dataset_path: str, query_filename: str, transform=None, transform_now=True):
        # assert os.path.exists(dataset_path), 'Cannot access dataset path: {}'.format(dataset_path)
        self.dataset_path = dataset_path
        self.transform = transform
        self.transform_now = transform_now
    ...
    def __getitem__(self, ndx):
        ...
        if self.transform is not None and self.transform_now:
            query_pc = self.transform(query_pc)
        
        mean = torch.tensor(self.queries[ndx].mean.reshape(1,3), dtype=torch.float)
        scale = torch.tensor(np.array(self.queries[ndx].scale).reshape(1,1), dtype=torch.float)

        return query_pc, ndx, mean, scale
...
class TrainingTuple:
    ...
    def __init__(self, id: int, timestamp: int, rel_scan_filepath: str, positives: np.ndarray,
                 non_negatives: np.ndarray, position: np.ndarray, mean=np.zeros([1, 3]), scale=1.0):
...
class TrainTransform:
    def __init__(self, aug_mode, fixed_theta=0.0):
        ...
        if self.aug_mode == 1:
            ...
            t = [FixedRotation(axis=np.array([0, 0, 1]), theta=fixed_theta)]
            self.transform = transforms.Compose(t)
        else:
...
class FixedRotation:
    def __init__(self, axis=None, theta=0):
        self.axis = axis
        self.theta = theta
    ...
    def __call__(self, coords):
        R = self._M(self.axis, np.pi * self.theta / 180)
        coords = coords @ R
