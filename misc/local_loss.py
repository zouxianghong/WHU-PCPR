...
import math
from misc.utils import pdist, hashM
...
class KeypointCorrLoss:
    ...
    def __call__(self, clouds1: torch.Tensor, keypoints1: torch.Tensor, sigma1: torch.Tensor, descriptors1: torch.Tensor,
        ...
        for i, (pc1, kp1_pos, s1, desc1, pc2, kp2_pos, s2, desc2) in enumerate(zip(clouds1, keypoints1, sigma1, descriptors1,
                                                                                clouds2, keypoints2, sigma2, descriptors2)):
            # NOTE: centralization is very important!
            center1 = torch.mean(pc1, dim=0, keepdims=True)
            pc1 = pc1 - center1
            kp1_pos = kp1_pos - center1
            pc2 = pc2 - center1
            kp2_pos = kp2_pos - center1
            
            dist_kp1_kp2 = torch.cdist(kp1_pos, kp2_pos)   # Euclidean distance

            metrics = {}
            metrics['kp_per_cloud'] = 0.5 * (len(kp1_pos) + len(kp2_pos))
            loss_keypoints, m = self.keypoint_loss(pc1, kp1_pos, s1, pc2, kp2_pos, s2, dist_kp1_kp2)
            metrics.update(m)

            loss_correspondence, m = self.correspondence_loss(desc1, desc2, dist_kp1_kp2)
...
class PointContrastiveLoss:
    def __init__(self, point_pos_margin=0.1, point_neg_margin=2.0, point_neg_weight=1.0,
                 num_pos=5192, num_hn_samples=2048):
        self.point_pos_margin = point_pos_margin
        self.point_neg_margin = point_neg_margin
        self.point_neg_weight = point_neg_weight
        self.num_pos = num_pos
        self.num_hn_samples = num_hn_samples
    
    def __call__(self, F0, F1, positive_pairs):
        """
        ...
        pos_loss = F.relu((posF0 - posF1).pow(2).sum(1) - self.point_pos_margin)
        neg_loss0 = F.relu(self.point_neg_margin - D01min[mask0]).pow(2)
        neg_loss1 = F.relu(self.point_neg_margin - D10min[mask1]).pow(2)

        pos_loss = pos_loss.mean()
        neg_loss = (neg_loss0.mean() + neg_loss1.mean()) / 2
        loss = pos_loss + self.point_neg_weight * neg_loss
        return loss
