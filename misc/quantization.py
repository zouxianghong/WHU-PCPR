...
def make_quantizer(quantization_type, quantization_size):
    assert quantization_type in ['polar', 'cartesian'], f'Unsupported coordinates: {quantization_type}'

    if 'polar' in quantization_type:
        # 3 quantization steps for polar coordinates: for sectors (in degrees), rings (in meters) and z coordinate (in meters)
        assert len(quantization_size) == 3, f'Expected 3 quantization sizes: for sectors (degrees), rings (meters) and z coordinate (meters)'
        quantizer = PolarQuantizer(quant_step=quantization_size)
    elif 'cartesian' in quantization_type:
        # Single quantization step for cartesian coordinates
        quantizer = CartesianQuantizer(quant_step=quantization_size)
    else:
        quantizer = None
    return quantizer
