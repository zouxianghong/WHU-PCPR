...
def sgv_parallel(src_keypts, tgt_keypts, src_features, tgt_features, d_thresh=5.0):
    """
    Input:
...
def sgv_fn(query_keypoints, tgt_lfs, tgt_kps, d_thresh=5.0, use_cpu=True):
    src_keypts = query_keypoints['keypoints'].unsqueeze(0)
    src_features = query_keypoints['features'].unsqueeze(0)
    if not use_cpu:
        src_keypts = src_keypts.cuda()
        src_features = src_features.cuda()
...
def sgv_fn_logg3dnet(query_keypoints, candidate_keypoints, d_thresh=5.0, max_points=15000, use_cpu=True):
    ...
    if not use_cpu:
        kp1 = kp1.unsqueeze(0).cuda()
        kp2 = kp2.unsqueeze(0).cuda()
        f1 = f1.unsqueeze(0).cuda()
        f2 = f2.unsqueeze(0).cuda()

    conf = sgv_parallel(kp1, kp2, f1, f2, d_thresh=d_thresh)
    return conf