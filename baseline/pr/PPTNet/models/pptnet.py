...
import loupe as lp
from baseline.pr.PPTNet.libs.pointops.functions import pointops
from baseline.pr.PPTNet.util import pt_util
...
class Network(nn.Module):
    def __init__(self, param=None):
        ...
        if param.aggregation == 'spvlad':
            ...
            print("No aggregation algorithm: ", param.aggregation)
    ...
    def forward(self, x):
        ...
        g_desc = self.aggregation(f0, f1, f2, f3)   # B x C0x64x1, BxC1x256, BxC2x1024, BxC3x4096 -> Bx256
        
        kpt2 = []
        x = x.unsqueeze(dim=1)
        for b_i in range(x.shape[0]):
            kpt_i = torch.index_select(x[b_i].squeeze(), dim=0, index=res['center_idx_origin'][0][b_i])
            kpt2.append(kpt_i)
        kpt2 = torch.stack(kpt2)
        f2 = f2.squeeze(-1).permute(0, 2, 1)
        
        return {'global':g_desc,  # B x 256
                'keypoints': kpt2,  # B x 1024 x 3
                'descriptors': f2  # B x 1024 x C
               }
...
class PointNet2SAModuleMSG(_PointNet2SAModuleBase):
    ...
    def __init__(self, *, npoint: int, radii: List[float], nsamples: List[int], mlps: List[List[int]], gp: int,
...
class PointNet2SAModule(PointNet2SAModuleMSG):
    ...
    def __init__(self, *, mlp: List[int], npoint: int = None, radius: float = None, nsample: int = None, gp: int = None,
                 bn: bool = True, use_xyz: bool = True):
