...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor]:
        """
        KNN Indexing (accelerated by cuda)
        input: nsample: int32, Number of neighbor
               xyz: (b, n, 3) coordinates of the features
               new_xyz: (b, m, 3) centriods
               output: idx: (b, m, nsample)
...
class QueryAndGroup_Edge(nn.Module):
    """
    Groups with a ball query of radius
    local feature - center feature
    parameters:
        radius: float32, Radius of ball
        nsample: int32, Maximum number of features to gather in the ball
    """
    def __init__(self, radius=None, nsample=32, knn_dilation=1, use_xyz=True, ret_gxyz=False, ret_sample_idx=False):
        super(QueryAndGroup_Edge, self).__init__()
        self.radius, self.nsample, self.knn_dilation, self.use_xyz = radius, nsample, knn_dilation, use_xyz
        self.ret_gxyz = ret_gxyz
        self.ret_sample_idx = ret_sample_idx

    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor = None, features: torch.Tensor = None, center_features: torch.Tensor = None, idx: torch.Tensor = None) -> \
    Tuple[Union[Tensor, Any], Any]:
        """
        input: xyz: (b, n, 3) coordinates of the features
               new_xyz: (b, m, 3) centriods
               features: (b, c, n)
               center_features: (b, c, m)
               idx: idx of neighbors
               # idxs: (b, n)
        output: new_features: (b, c+3, m, nsample)
        ...
        if idx is None:
            if self.radius is not None:
                ...
                if self.knn_dilation > 1:
                    # idx = knnquery_naive(self.nsample, xyz, new_xyz)   # (b, m, nsample)
                    candidates_idx = knnquery(self.knn_dilation * self.nsample, xyz, new_xyz)  # (b, m, knn_dilation * nsample), accelerated by cuda
                    idx = torch.cuda.IntTensor(candidates_idx.size(0), candidates_idx.size(1), self.nsample)
                    idx[:, :] = candidates_idx[:, :, torch.randperm(self.nsample)]
                else:
                    # idx = knnquery_naive(self.nsample, xyz, new_xyz)   # (b, m, nsample)
                    idx = knnquery(self.nsample, xyz, new_xyz)  # (b, m, nsample), accelerated by cuda
        xyz_trans = xyz.transpose(1, 2).contiguous()  # b x 3 x n
        o_grouped_xyz = grouping(xyz_trans, idx)  # (b, 3, m, nsample)

        grouped_xyz = o_grouped_xyz - new_xyz.transpose(1, 2).unsqueeze(-1)  # centralize
        if features is not None:
            grouped_features = grouping(features, idx)
            # grouped_features: BxCxNxK
            # center_features: BxCxN
            if grouped_features.size(3) > 1:
                grouped_features = grouped_features - center_features.unsqueeze(-1)  # centralize
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz, grouped_features], dim=1)    # (b, c+3, m, nsample)
            else:
                new_features = grouped_features
        ...
        res = new_features
        if self.ret_gxyz:
            res = res, o_grouped_xyz
        if self.ret_sample_idx:
            res = res, idx
        return res

class QueryAndGroup_Edge_Split(nn.Module):
    """
    Groups with a ball query of radius
    le: local feature - center feature
    parameters:
        radius: float32, Radius of ball
        nsample: int32, Maximum number of features to gather in the ball
    """
    def __init__(self, radius=None, nsample=32, use_xyz=True, ret_gxyz=False):
        super(QueryAndGroup_Edge_Split, self).__init__()
        self.radius, self.nsample, self.use_xyz = radius, nsample, use_xyz
        self.ret_gxyz = ret_gxyz

    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor = None, features: torch.Tensor = None, center_features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        """
        input: xyz: (b, n, 3) coordinates of the features
               new_xyz: (b, m, 3) centriods
               features: (b, c, n)
        ...
        grouped_xyz = o_grouped_xyz - new_xyz.transpose(1, 2).unsqueeze(-1)
        if features is not None:
            grouped_features = grouping(features, idx)
            # grouped_features: BxCxNxK
            # center_features: BxCxN
        ...
        if self.ret_gxyz == True:
            return new_features, o_grouped_xyz
        else:
